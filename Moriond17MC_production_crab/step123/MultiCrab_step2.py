...
def prepare(dataset):
    os.system('rm -rf step2_inputdataset.txt')
    f = open(dataset,'r')
    for line in f:
        print line
        a=[]
        ...
        datasetdetail=[a,b]
        print datasetdetail
        name='crab status -d crab_projects_MonoHStep1/crab_step1-'+datasetdetail[1]+' | grep -a "Output dataset:" | awk -v my_var='+datasetdetail[1]+' \'{print $3,my_var}\'>> step2_inputdataset.txt'
        print name
        os.system(name)
...
if sys.argv[1] == "prepare" :
    if len(sys.argv) != 3 :
        print "Usuage: python MultiCrab_step2.py prepare step1_inputdataset.txt"
        exit (1)
    else:
