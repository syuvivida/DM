...
mh4=(100)
mdm=(100)
workdir=/afs/cern.ch/work/s/syu/HHMET_study/MG5_aMC_v3_5_7/type2_visible_h4h1
cd $workdir

echo "MA" "BR(a->b+b~)" "Xsec" "effective Xsec (Xsec*BR)" >> $xsecDataFile
for h2 in "${mh2[@]}"
do
    seth2=$h2
    for h4 in "${mh4[@]}"
	...
	do	    
	    ...
	    echo $decaya
	    finalXSec=`awk -v a=$xsec -v d=$decaya 'BEGIN{print (a*d)}'`
	    echo $finalXSec
	    echo $seth2 $seth4 $setdm $decaya $xsec $finalXSec >> $xsecDataFile
	    olddir=${dir}_decayed_1
	    newdir=associated_ah_abb_13p6TeV_mdm_${setdm}_MA_${seth2}_Ma_${seth4}
	    echo $olddir $newdir
