...
echo "sinp" "lambda3" "widthH" "MH" "widthA" "MA" "FracWH" "FracWA" >> $widthDataFile
for setsinp in "${sinp[@]}"
do
    for setl3 in "${lam3[@]}"
    do
	...
	echo $setsinp $setl3 $widthH $massH $widthA $massA $FracWH $FracWA >> $widthDataFile
	
	olddir=`ls -lrt Events | tail -1 | awk '{print $9}'`
	roundsinp=`printf "%.1f" $setsinp`
	roundsinp=${roundsinp/\./p}
	roundMA=`printf "%.0f" $massA`
	roundMH=`printf "%.0f" $massH`
	newdir=resonant_HAAah_13p6TeV_MA_${roundMA}_MH_${roundMH}_tanbeta2_lambda3_${setl3}_sint${roundsinp}
	echo $olddir $newdir
